...
[GitHub Security Advisories](https://github.com/tothienbao6a0/aq/security/advisories/new)
rather than as a public issue. Expect a first response within a week.

## What is in scope

`@aq/core` does no I/O — no filesystem, no network, no process spawning — so
...
- **`@aq/adapter-local`** spawns a player process with a path or URL from a
  queue entry. Argument injection or path traversal through a crafted entry is
  in scope.
- **`@aq/adapter-process`** runs a command the host configures and speaks JSON
...
- The `resolveIntent` callback. It is host-supplied arbitrary code, and it is
  meant to be.
- Anything under `@aq/core/internal`, which is unsupported.
