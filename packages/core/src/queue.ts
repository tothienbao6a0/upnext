...
export class Queue {
  ...
  /** Live references. Internal callers only — mutating these skips versioning. */
  raw(): readonly QueueItem[] {
    return this.#items;
  }
  ...
  nextPlayable(fromId: string | null): QueueItem | undefined {
    ...
    for (let i = Math.max(0, start); i < this.#items.length; i++) {
      ...
      if (isPlayableStatus(item.status)) return item;
    }
    return undefined;
  }
  ...
  clear(opts: { keepActive?: boolean } = {}): void {
    ...
    this.#items = keepActive
      ? this.#items.filter((item) => item.id === this.cursorId)
      : [];
